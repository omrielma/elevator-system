...
export let floorArr = [0, 0, 0, 0, 0];

const chooseLift = (idx) => {
  switch(idx){
    case 0:
      return ".person-1";
    case 1:
...
export const closestLift = (flr) => {
  let min = 10;
  let ret;
  floorArr.forEach((e) => {
    if (Math.abs(flr-e) < min && e != null){
      min = Math.abs(flr-e);
      ret = e;
    }
  });
...
class Lift extends Component{
  ...
  async shouldComponentUpdate(nextState){
    if(this.state.className === "person-5"){
      if(floorArr.every((e) => e === null)){
        setTimeout(() => this.shouldComponentUpdate(nextState), 50)
      }else{
      this.state.floor = nextState.floor;
      const flr = this.state.floor;
      const close = closestLift(Number(flr));
      const liftIndex = floorArr.indexOf(close);
      floorArr[liftIndex] = null;
      const eleNum = chooseLift(liftIndex);
      const doc = document.querySelector(eleNum);
      doc.style.bottom = this.moveElevator(nextState.floor);
      const svg = document.getElementById(eleNum);
      svg.style.fill = "red";
      setTimeout(() => svg.style.fill = 'green', 13000);
      setTimeout(() => {floorArr[liftIndex] = Number(flr); svg.style.fill = "black"}, 16000);
      }
    } 
  }
